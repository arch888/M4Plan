...
def styleorder(request):
    if (request.method == 'POST'):
        form = Styleorders(request.POST)
        if form.is_valid():
            form.save()
            return HttpResponse("<h1>Style order submitted</h1>")
        else:
            messages.error(request, "Error")
        ...
        form = Styleorders()
    return render(request, 'stylemaster/style_order.html', {'form': form})

def dashorder(request):
    a=StyleOrder.objects.all()
    return render(request, 'stylemaster/dash_styleorder.html',{'a':a})

def garmentitem(request):
    if (request.method == 'POST'):
        form = Garmentitem(request.POST)
        if form.is_valid():
            form.save()
            return HttpResponse("<h1>Garment Item submitted</h1>")
        else:
            messages.error(request, "Error")
        ...
        form = Garmentitem()
    return render(request, 'stylemaster/garmentitem.html', {'form': form})

def dashgarment(request):
    a=GarmentItem.objects.all()
    return render(request, 'stylemaster/dash_garmentitem.html',{'a':a})
