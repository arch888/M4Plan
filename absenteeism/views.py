...
def register(request):
    if (request.method=='POST'):
        ...
        if (form.is_valid()):
            ...
            #user.employee.choose=form.cleaned_data-['choose']
            #user.save()
            a.user=User.objects.get(username=form.cleaned_data['username'])
            a.desig=form.cleaned_data['desig']
            a.choose=Lining.objects.get(select=form.cleaned_data['choose'])
            a.save()
            user.save()
...
def profile(request):
    if (request.method == 'POST'):
        ...
        if (form.is_valid()):
            ...
            e=Employee.objects.get(user__username=request.user.username)
            obj.choosen=e.choose
            a = Person.objects.filter(name=request.user.username)
            t = str(obj.date)
            c = 0
            for i in a:
                if (str(i.date) == t):
...
def check(request):
    a=Person.objects.filter(name=request.user.username).order_by('-date')
    l=[]
    k=[]
    for i in a:
        l.append((i.date))
...
def line(request):
    if(request.method=='POST'):
        ...
        if(form.is_valid()):
            ...
            a=Person.objects.filter(choose__select=data)
            for i in a:
                s.add(i.name)
            return render(request,'Absenteeism/line.html',{'value':s})
        else:
            messages.error(request, "Error")
    else:
        form=My()
    return render(request,'Absenteeism/lining.html',{'form':form})
